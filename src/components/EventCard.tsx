    ...
    >
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-0 p-4">
          ...
          {!hovered && !isWhatsappLead && price !== null && (
            <p className="mt-1.5 font-serif text-sm font-bold text-amber-400">{formatBRL(price)}</p>
          )}
          {!hovered && isWhatsappLead && (
            <p className="mt-1.5 flex items-center gap-1 text-[10px] font-semibold text-green-400">
      ...
      >
        <div className="p-4 space-y-3">
          ...
          <p className="text-[11px] leading-relaxed text-gray-400 line-clamp-3">{description}</p>

          <div className="flex items-center gap-2 flex-wrap">
            {formattedDate && (
              <span className="rounded bg-zinc-800 px-2 py-1 text-[10px] text-gray-400">
                📅 {formattedDate}
              </span>
            )}
            {maxSlots && (
